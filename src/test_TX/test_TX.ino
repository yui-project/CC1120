...
void loop() {
  ...
  for(int i=0; i<4; i++){
    ...
    // strobeSPI(SWOR);  // Enable TX
    delay(10);
    strobeSPI(STX);
    delay(10);
    writeSPI(TXRX_FIFO, buffer[0]);
  
    // Serial.println(readExtAddrSPI(MARC_STATUS0), BIN);
    // Serial.println(readExtAddrSPI(MARC_STATUS1), BIN);
    // Serial.println(readExtAddrSPI(MODEM_STATUS0), BIN);
    // Serial.println(readExtAddrSPI(MODEM_STATUS1), BIN);
    // Serial.println(readExtAddrSPI(SERIAL_STATUS), BIN);
    // Serial.print("MARCSTATE before delay:  ");
    // readMARCSTATE();
    delay(3000);
...
writeExtAddrSPI(IF_MIX_CFG, IF_MIX_CFG_VALUE);
