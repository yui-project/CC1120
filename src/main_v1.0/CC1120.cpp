...
bool CC1120Class::setFSK(){
  ...
  ret = (readSPI(IOCFG3)           == FSK_IOCFG3_VALUE              );
  ret = (readSPI(IOCFG2)           == FSK_IOCFG2_VALUE              );
  ret = (readSPI(IOCFG1)           == FSK_IOCFG1_VALUE              );
  ret = (readSPI(IOCFG0)           == FSK_IOCFG0_VALUE              );
  ret = (readSPI(SYNC3)            == FSK_SYNC3_VALUE               );
  ret = (readSPI(SYNC2)            == FSK_SYNC2_VALUE               );
  ret = (readSPI(SYNC1)            == FSK_SYNC1_VALUE               );
  ret = (readSPI(SYNC0)            == FSK_SYNC0_VALUE               );
  ret = (readSPI(SYNC_CFG1)        == FSK_SYNC_CFG1_VALUE           );
  ret = (readSPI(SYNC_CFG0)        == FSK_SYNC_CFG0_VALUE           );
  ret = (readSPI(DEVIATION_M)      == FSK_DEVIATION_M_VALUE         );
  ret = (readSPI(MODCFG_DEV_E)     == FSK_MODCFG_DEV_E_VALUE        );
  ret = (readSPI(DCFILT_CFG)       == FSK_DCFILT_CFG_VALUE          );
  ret = (readSPI(PREAMBLE_CFG1)    == FSK_PREAMBLE_CFG1_VALUE       );
  ret = (readSPI(PREAMBLE_CFG0)    == FSK_PREAMBLE_CFG0_VALUE       );
  ret = (readSPI(FREQ_IF_CFG)      == FSK_FREQ_IF_CFG_VALUE         );
  ret = (readSPI(IQIC)             == FSK_IQIC_VALUE                );
  ret = (readSPI(CHAN_BW)          == FSK_CHAN_BW_VALUE             );
  ret = (readSPI(MDMCFG1)          == FSK_MDMCFG1_VALUE             );
  ret = (readSPI(MDMCFG0)          == FSK_MDMCFG0_VALUE             );
  ret = (readSPI(SYMBOL_RATE2)     == FSK_SYMBOL_RATE2_VALUE        );
  ret = (readSPI(SYMBOL_RATE1)     == FSK_SYMBOL_RATE1_VALUE        );
  ret = (readSPI(SYMBOL_RATE0)     == FSK_SYMBOL_RATE0_VALUE        );
  ret = (readSPI(AGC_REF)          == FSK_AGC_REF_VALUE             );
  ret = (readSPI(AGC_CS_THR)       == FSK_AGC_CS_THR_VALUE          );
  ret = (readSPI(AGC_GAIN_ADJUST)  == FSK_AGC_GAIN_ADJUST_VALUE     );
  ret = (readSPI(AGC_CFG3)         == FSK_AGC_CFG3_VALUE            );
  ret = (readSPI(AGC_CFG2)         == FSK_AGC_CFG2_VALUE            );
  ret = (readSPI(AGC_CFG1)         == FSK_AGC_CFG1_VALUE            );
  ret = (readSPI(AGC_CFG0)         == FSK_AGC_CFG0_VALUE            );
  ret = (readSPI(FIFO_CFG)         == FSK_FIFO_CFG_VALUE            );
  ret = (readSPI(DEV_ADDR)         == FSK_DEV_ADDR_VALUE            );
  ret = (readSPI(SETTLING_CFG)     == FSK_SETTLING_CFG_VALUE        );
  ret = (readSPI(FS_CFG)           == FSK_FS_CFG_VALUE              );
  ret = (readSPI(WOR_CFG1)         == FSK_WOR_CFG1_VALUE            );
  ret = (readSPI(WOR_CFG0)         == FSK_WOR_CFG0_VALUE            );
  ret = (readSPI(WOR_EVENT0_MSB)   == FSK_WOR_EVENT0_MSB_VALUE      );
  ret = (readSPI(WOR_EVENT0_LSB)   == FSK_WOR_EVENT0_LSB_VALUE      );
  ret = (readSPI(PKT_CFG2)         == FSK_PKT_CFG2_VALUE            );
  ret = (readSPI(PKT_CFG1)         == FSK_PKT_CFG1_VALUE            );
  ret = (readSPI(PKT_CFG0)         == FSK_PKT_CFG0_VALUE            );
  ret = (readSPI(RFEND_CFG1)       == FSK_RFEND_CFG1_VALUE          );
  ret = (readSPI(RFEND_CFG0)       == FSK_RFEND_CFG0_VALUE          );
  ret = (readSPI(PA_CFG2)          == FSK_PA_CFG2_VALUE             );
  ret = (readSPI(PA_CFG1)          == FSK_PA_CFG1_VALUE             );
  ret = (readSPI(PA_CFG0)          == FSK_PA_CFG0_VALUE             );
  ret = (readSPI(PKT_LEN)          == FSK_PKT_LEN_VALUE             );

  Serial.print(".");

  ret = (readExtAddrSPI(IF_MIX_CFG)       == FSK_IF_MIX_CFG_VALUE          );
  ret = (readExtAddrSPI(FREQOFF_CFG)      == FSK_FREQOFF_CFG_VALUE         );
  ret = (readExtAddrSPI(TOC_CFG)          == FSK_TOC_CFG_VALUE             );
  ret = (readExtAddrSPI(MARC_SPARE)       == FSK_MARC_SPARE_VALUE          );
  ret = (readExtAddrSPI(ECG_CFG)          == FSK_ECG_CFG_VALUE             );
  ret = (readExtAddrSPI(CFM_DATA_CFG)     == FSK_CFM_DATA_CFG_VALUE        );
  ret = (readExtAddrSPI(EXT_CTRL)         == FSK_EXT_CTRL_VALUE            );
  ret = (readExtAddrSPI(RCCAL_FINE)       == FSK_RCCAL_FINE_VALUE          );
  ret = (readExtAddrSPI(RCCAL_COARSE)     == FSK_RCCAL_COARSE_VALUE        );
  ret = (readExtAddrSPI(RCCAL_OFFSET)     == FSK_RCCAL_OFFSET_VALUE        );
  ret = (readExtAddrSPI(FREQOFF1)         == FSK_FREQOFF1_VALUE            );
  ret = (readExtAddrSPI(FREQOFF0)         == FSK_FREQOFF0_VALUE            );
  ret = (readExtAddrSPI(FREQ2)            == FSK_FREQ2_VALUE               );
  ret = (readExtAddrSPI(FREQ1)            == FSK_FREQ1_VALUE               );
  ret = (readExtAddrSPI(FREQ0)            == FSK_FREQ0_VALUE               );
  ret = (readExtAddrSPI(IF_ADC2)          == FSK_IF_ADC2_VALUE             );
  ret = (readExtAddrSPI(IF_ADC1)          == FSK_IF_ADC1_VALUE             );
  ret = (readExtAddrSPI(IF_ADC0)          == FSK_IF_ADC0_VALUE             );
  ret = (readExtAddrSPI(FS_DIG1)          == FSK_FS_DIG1_VALUE             );
  ret = (readExtAddrSPI(FS_DIG0)          == FSK_FS_DIG0_VALUE             );
  ret = (readExtAddrSPI(FS_CAL3)          == FSK_FS_CAL3_VALUE             );
  ret = (readExtAddrSPI(FS_CAL2)          == FSK_FS_CAL2_VALUE             );
  ret = (readExtAddrSPI(FS_CAL1)          == FSK_FS_CAL1_VALUE             );
  ret = (readExtAddrSPI(FS_CAL0)          == FSK_FS_CAL0_VALUE             );
  ret = (readExtAddrSPI(FS_CHP)           == FSK_FS_CHP_VALUE              );
  ret = (readExtAddrSPI(FS_DIVTWO)        == FSK_FS_DIVTWO_VALUE           );
  ret = (readExtAddrSPI(FS_DSM1)          == FSK_FS_DSM1_VALUE             );
  ret = (readExtAddrSPI(FS_DSM0)          == FSK_FS_DSM0_VALUE             );
  ret = (readExtAddrSPI(FS_DVC1)          == FSK_FS_DVC1_VALUE             );
  ret = (readExtAddrSPI(FS_DVC0)          == FSK_FS_DVC0_VALUE             );
  ret = (readExtAddrSPI(FS_LBI)           == FSK_FS_LBI_VALUE              );
  ret = (readExtAddrSPI(FS_PFD)           == FSK_FS_PFD_VALUE              );
  ret = (readExtAddrSPI(FS_PRE)           == FSK_FS_PRE_VALUE              );
  ret = (readExtAddrSPI(FS_REG_DIV_CML)   == FSK_FS_REG_DIV_CML_VALUE      );
  ret = (readExtAddrSPI(FS_SPARE)         == FSK_FS_SPARE_VALUE            );
  ret = (readExtAddrSPI(FS_VCO4)          == FSK_FS_VCO4_VALUE             );
  ret = (readExtAddrSPI(FS_VCO3)          == FSK_FS_VCO3_VALUE             );
  ret = (readExtAddrSPI(FS_VCO2)          == FSK_FS_VCO2_VALUE             );
  ret = (readExtAddrSPI(FS_VCO1)          == FSK_FS_VCO1_VALUE             );
  ret = (readExtAddrSPI(FS_VCO0)          == FSK_FS_VCO0_VALUE             );
  ret = (readExtAddrSPI(GBIAS6)           == FSK_GBIAS6_VALUE              );
  ret = (readExtAddrSPI(GBIAS5)           == FSK_GBIAS5_VALUE              );
  ret = (readExtAddrSPI(GBIAS4)           == FSK_GBIAS4_VALUE              );
  ret = (readExtAddrSPI(GBIAS3)           == FSK_GBIAS3_VALUE              );
  ret = (readExtAddrSPI(GBIAS2)           == FSK_GBIAS2_VALUE              );
  ret = (readExtAddrSPI(GBIAS1)           == FSK_GBIAS1_VALUE              );
  ret = (readExtAddrSPI(GBIAS0)           == FSK_GBIAS0_VALUE              );
  ret = (readExtAddrSPI(IFAMP)            == FSK_IFAMP_VALUE               );
  ret = (readExtAddrSPI(LNA)              == FSK_LNA_VALUE                 );
  ret = (readExtAddrSPI(RXMIX)            == FSK_RXMIX_VALUE               );
  ret = (readExtAddrSPI(XOSC5)            == FSK_XOSC5_VALUE               );
  ret = (readExtAddrSPI(XOSC4)            == FSK_XOSC4_VALUE               );
  ret = (readExtAddrSPI(XOSC3)            == FSK_XOSC3_VALUE               );
  ret = (readExtAddrSPI(XOSC2)            == FSK_XOSC2_VALUE               );
  ret = (readExtAddrSPI(XOSC1)            == FSK_XOSC1_VALUE               );
  ret = (readExtAddrSPI(XOSC0)            == FSK_XOSC0_VALUE               );
  ret = (readExtAddrSPI(ANALOG_SPARE)     == FSK_ANALOG_SPARE_VALUE        );
  ret = (readExtAddrSPI(PA_CFG3)          == FSK_PA_CFG3_VALUE             );
  ret = (readExtAddrSPI(WOR_TIME1)        == FSK_WOR_TIME1_VALUE           );
  ret = (readExtAddrSPI(WOR_TIME0)        == FSK_WOR_TIME0_VALUE           );
  ret = (readExtAddrSPI(WOR_CAPTURE1)     == FSK_WOR_CAPTURE1_VALUE        );
  ret = (readExtAddrSPI(WOR_CAPTURE0)     == FSK_WOR_CAPTURE0_VALUE        );
  ret = (readExtAddrSPI(BIST)             == FSK_BIST_VALUE                );
  ret = (readExtAddrSPI(DCFILTOFFSET_I1)  == FSK_DCFILTOFFSET_I1_VALUE     );
  ret = (readExtAddrSPI(DCFILTOFFSET_I0)  == FSK_DCFILTOFFSET_I0_VALUE     );
  ret = (readExtAddrSPI(DCFILTOFFSET_Q1)  == FSK_DCFILTOFFSET_Q1_VALUE     );
  ret = (readExtAddrSPI(DCFILTOFFSET_Q0)  == FSK_DCFILTOFFSET_Q0_VALUE     );
  ret = (readExtAddrSPI(IQIE_I1)          == FSK_IQIE_I1_VALUE             );
  ret = (readExtAddrSPI(IQIE_I0)          == FSK_IQIE_I0_VALUE             );
  ret = (readExtAddrSPI(IQIE_Q1)          == FSK_IQIE_Q1_VALUE             );
  ret = (readExtAddrSPI(IQIE_Q0)          == FSK_IQIE_Q0_VALUE             );
  ret = (readExtAddrSPI(RSSI1)            == FSK_RSSI1_VALUE               );
  ret = (readExtAddrSPI(RSSI0)            == FSK_RSSI0_VALUE               );
  ret = (readExtAddrSPI(MARCSTATE)        == FSK_MARCSTATE_VALUE           );
  ret = (readExtAddrSPI(LQI_VAL)          == FSK_LQI_VAL_VALUE             );
  ret = (readExtAddrSPI(PQT_SYNC_ERR)     == FSK_PQT_SYNC_ERR_VALUE        );
  ret = (readExtAddrSPI(DEM_STATUS)       == FSK_DEM_STATUS_VALUE          );
  ret = (readExtAddrSPI(FREQOFF_EST1)     == FSK_FREQOFF_EST1_VALUE        );
  ret = (readExtAddrSPI(FREQOFF_EST0)     == FSK_FREQOFF_EST0_VALUE        );
  ret = (readExtAddrSPI(AGC_GAIN3)        == FSK_AGC_GAIN3_VALUE           );
  ret = (readExtAddrSPI(AGC_GAIN2)        == FSK_AGC_GAIN2_VALUE           );
  ret = (readExtAddrSPI(AGC_GAIN1)        == FSK_AGC_GAIN1_VALUE           );
  ret = (readExtAddrSPI(AGC_GAIN0)        == FSK_AGC_GAIN0_VALUE           );
  ret = (readExtAddrSPI(CFM_RX_DATA_OUT)  == FSK_CFM_RX_DATA_OUT_VALUE     );
  ret = (readExtAddrSPI(CFM_TX_DATA_IN)   == FSK_CFM_TX_DATA_IN_VALUE      );
  ret = (readExtAddrSPI(ASK_SOFT_RX_DATA) == FSK_ASK_SOFT_RX_DATA_VALUE    );
  ret = (readExtAddrSPI(RNDGEN)           == FSK_RNDGEN_VALUE              );
  ret = (readExtAddrSPI(MAGN2)            == FSK_MAGN2_VALUE               );
  ret = (readExtAddrSPI(MAGN1)            == FSK_MAGN1_VALUE               );
  ret = (readExtAddrSPI(MAGN0)            == FSK_MAGN0_VALUE               );
  ret = (readExtAddrSPI(ANG1)             == FSK_ANG1_VALUE                );
  ret = (readExtAddrSPI(ANG0)             == FSK_ANG0_VALUE                );
  ret = (readExtAddrSPI(CHFILT_I2)        == FSK_CHFILT_I2_VALUE           );
  ret = (readExtAddrSPI(CHFILT_I1)        == FSK_CHFILT_I1_VALUE           );
  ret = (readExtAddrSPI(CHFILT_I0)        == FSK_CHFILT_I0_VALUE           );
  ret = (readExtAddrSPI(CHFILT_Q2)        == FSK_CHFILT_Q2_VALUE           );
  ret = (readExtAddrSPI(CHFILT_Q1)        == FSK_CHFILT_Q1_VALUE           );
  ret = (readExtAddrSPI(CHFILT_Q0)        == FSK_CHFILT_Q0_VALUE           );
  ret = (readExtAddrSPI(GPIO_STATUS)      == FSK_GPIO_STATUS_VALUE         );
  ret = (readExtAddrSPI(FSCAL_CTRL)       == FSK_FSCAL_CTRL_VALUE          );
  ret = (readExtAddrSPI(PHASE_ADJUST)     == FSK_PHASE_ADJUST_VALUE        );
  ret = (readExtAddrSPI(SERIAL_STATUS)    == FSK_SERIAL_STATUS_VALUE       );
  ret = (readExtAddrSPI(MODEM_STATUS1)    == FSK_MODEM_STATUS1_VALUE       );
  ret = (readExtAddrSPI(MODEM_STATUS0)    == FSK_MODEM_STATUS0_VALUE       );
  ret = (readExtAddrSPI(MARC_STATUS1)     == FSK_MARC_STATUS1_VALUE        );
  ret = (readExtAddrSPI(MARC_STATUS0)     == FSK_MARC_STATUS0_VALUE        );
  ret = (readExtAddrSPI(PA_IFAMP_TEST)    == FSK_PA_IFAMP_TEST_VALUE       );
  ret = (readExtAddrSPI(FSRF_TEST)        == FSK_FSRF_TEST_VALUE           );
  ret = (readExtAddrSPI(PRE_TEST)         == FSK_PRE_TEST_VALUE            );
  ret = (readExtAddrSPI(PRE_OVR)          == FSK_PRE_OVR_VALUE             );
  ret = (readExtAddrSPI(ADC_TEST)         == FSK_ADC_TEST_VALUE            );
  ret = (readExtAddrSPI(DVC_TEST)         == FSK_DVC_TEST_VALUE            );
  ret = (readExtAddrSPI(ATEST)            == FSK_ATEST_VALUE               );
  ret = (readExtAddrSPI(ATEST_LVDS)       == FSK_ATEST_LVDS_VALUE          );
  ret = (readExtAddrSPI(ATEST_MODE)       == FSK_ATEST_MODE_VALUE          );
  ret = (readExtAddrSPI(XOSC_TEST1)       == FSK_XOSC_TEST1_VALUE          );
  ret = (readExtAddrSPI(XOSC_TEST0)       == FSK_XOSC_TEST0_VALUE          );
  Serial.println(".");
...
bool CC1120Class::setCW(){
  ...
  ret = (readSPI(IOCFG3)           == CW_IOCFG3_VALUE              );
  ret = (readSPI(IOCFG2)           == CW_IOCFG2_VALUE              );
  ret = (readSPI(IOCFG1)           == CW_IOCFG1_VALUE              );
  ret = (readSPI(IOCFG0)           == CW_IOCFG0_VALUE              );
  ret = (readSPI(SYNC3)            == CW_SYNC3_VALUE               );
  ret = (readSPI(SYNC2)            == CW_SYNC2_VALUE               );
  ret = (readSPI(SYNC1)            == CW_SYNC1_VALUE               );
  ret = (readSPI(SYNC0)            == CW_SYNC0_VALUE               );
  ret = (readSPI(SYNC_CFG1)        == CW_SYNC_CFG1_VALUE           );
  ret = (readSPI(SYNC_CFG0)        == CW_SYNC_CFG0_VALUE           );
  ret = (readSPI(DEVIATION_M)      == CW_DEVIATION_M_VALUE         );
  ret = (readSPI(MODCFG_DEV_E)     == CW_MODCFG_DEV_E_VALUE        );
  ret = (readSPI(DCFILT_CFG)       == CW_DCFILT_CFG_VALUE          );
  ret = (readSPI(PREAMBLE_CFG1)    == CW_PREAMBLE_CFG1_VALUE       );
  ret = (readSPI(PREAMBLE_CFG0)    == CW_PREAMBLE_CFG0_VALUE       );
  ret = (readSPI(FREQ_IF_CFG)      == CW_FREQ_IF_CFG_VALUE         );
  ret = (readSPI(IQIC)             == CW_IQIC_VALUE                );
  ret = (readSPI(CHAN_BW)          == CW_CHAN_BW_VALUE             );
  ret = (readSPI(MDMCFG1)          == CW_MDMCFG1_VALUE             );
  ret = (readSPI(MDMCFG0)          == CW_MDMCFG0_VALUE             );
  ret = (readSPI(SYMBOL_RATE2)     == CW_SYMBOL_RATE2_VALUE        );
  ret = (readSPI(SYMBOL_RATE1)     == CW_SYMBOL_RATE1_VALUE        );
  ret = (readSPI(SYMBOL_RATE0)     == CW_SYMBOL_RATE0_VALUE        );
  ret = (readSPI(AGC_REF)          == CW_AGC_REF_VALUE             );
  ret = (readSPI(AGC_CS_THR)       == CW_AGC_CS_THR_VALUE          );
  ret = (readSPI(AGC_GAIN_ADJUST)  == CW_AGC_GAIN_ADJUST_VALUE     );
  ret = (readSPI(AGC_CFG3)         == CW_AGC_CFG3_VALUE            );
  ret = (readSPI(AGC_CFG2)         == CW_AGC_CFG2_VALUE            );
  ret = (readSPI(AGC_CFG1)         == CW_AGC_CFG1_VALUE            );
  ret = (readSPI(AGC_CFG0)         == CW_AGC_CFG0_VALUE            );
  ret = (readSPI(FIFO_CFG)         == CW_FIFO_CFG_VALUE            );
  ret = (readSPI(DEV_ADDR)         == CW_DEV_ADDR_VALUE            );
  ret = (readSPI(SETTLING_CFG)     == CW_SETTLING_CFG_VALUE        );
  ret = (readSPI(FS_CFG)           == CW_FS_CFG_VALUE              );
  ret = (readSPI(WOR_CFG1)         == CW_WOR_CFG1_VALUE            );
  ret = (readSPI(WOR_CFG0)         == CW_WOR_CFG0_VALUE            );
  ret = (readSPI(WOR_EVENT0_MSB)   == CW_WOR_EVENT0_MSB_VALUE      );
  ret = (readSPI(WOR_EVENT0_LSB)   == CW_WOR_EVENT0_LSB_VALUE      );
  ret = (readSPI(PKT_CFG2)         == CW_PKT_CFG2_VALUE            );
  ret = (readSPI(PKT_CFG1)         == CW_PKT_CFG1_VALUE            );
  ret = (readSPI(PKT_CFG0)         == CW_PKT_CFG0_VALUE            );
  ret = (readSPI(RFEND_CFG1)       == CW_RFEND_CFG1_VALUE          );
  ret = (readSPI(RFEND_CFG0)       == CW_RFEND_CFG0_VALUE          );
  ret = (readSPI(PA_CFG2)          == CW_PA_CFG2_VALUE             );
  ret = (readSPI(PA_CFG1)          == CW_PA_CFG1_VALUE             );
  ret = (readSPI(PA_CFG0)          == CW_PA_CFG0_VALUE             );
  ret = (readSPI(PKT_LEN)          == CW_PKT_LEN_VALUE             );



  ret = (readExtAddrSPI(IF_MIX_CFG)       == CW_IF_MIX_CFG_VALUE          );
  ret = (readExtAddrSPI(FREQOFF_CFG)      == CW_FREQOFF_CFG_VALUE         );
  ret = (readExtAddrSPI(TOC_CFG)          == CW_TOC_CFG_VALUE             );
  ret = (readExtAddrSPI(MARC_SPARE)       == CW_MARC_SPARE_VALUE          );
  ret = (readExtAddrSPI(ECG_CFG)          == CW_ECG_CFG_VALUE             );
  ret = (readExtAddrSPI(CFM_DATA_CFG)     == CW_CFM_DATA_CFG_VALUE        );
  ret = (readExtAddrSPI(EXT_CTRL)         == CW_EXT_CTRL_VALUE            );
  ret = (readExtAddrSPI(RCCAL_FINE)       == CW_RCCAL_FINE_VALUE          );
  ret = (readExtAddrSPI(RCCAL_COARSE)     == CW_RCCAL_COARSE_VALUE        );
  ret = (readExtAddrSPI(RCCAL_OFFSET)     == CW_RCCAL_OFFSET_VALUE        );
  ret = (readExtAddrSPI(FREQOFF1)         == CW_FREQOFF1_VALUE            );
  ret = (readExtAddrSPI(FREQOFF0)         == CW_FREQOFF0_VALUE            );
  ret = (readExtAddrSPI(FREQ2)            == CW_FREQ2_VALUE               );
  ret = (readExtAddrSPI(FREQ1)            == CW_FREQ1_VALUE               );
  ret = (readExtAddrSPI(FREQ0)            == CW_FREQ0_VALUE               );
  ret = (readExtAddrSPI(IF_ADC2)          == CW_IF_ADC2_VALUE             );
  ret = (readExtAddrSPI(IF_ADC1)          == CW_IF_ADC1_VALUE             );
  ret = (readExtAddrSPI(IF_ADC0)          == CW_IF_ADC0_VALUE             );
  ret = (readExtAddrSPI(FS_DIG1)          == CW_FS_DIG1_VALUE             );
  ret = (readExtAddrSPI(FS_DIG0)          == CW_FS_DIG0_VALUE             );
  ret = (readExtAddrSPI(FS_CAL3)          == CW_FS_CAL3_VALUE             );
  ret = (readExtAddrSPI(FS_CAL2)          == CW_FS_CAL2_VALUE             );
  ret = (readExtAddrSPI(FS_CAL1)          == CW_FS_CAL1_VALUE             );
  ret = (readExtAddrSPI(FS_CAL0)          == CW_FS_CAL0_VALUE             );
  ret = (readExtAddrSPI(FS_CHP)           == CW_FS_CHP_VALUE              );
  ret = (readExtAddrSPI(FS_DIVTWO)        == CW_FS_DIVTWO_VALUE           );
  ret = (readExtAddrSPI(FS_DSM1)          == CW_FS_DSM1_VALUE             );
  ret = (readExtAddrSPI(FS_DSM0)          == CW_FS_DSM0_VALUE             );
  ret = (readExtAddrSPI(FS_DVC1)          == CW_FS_DVC1_VALUE             );
  ret = (readExtAddrSPI(FS_DVC0)          == CW_FS_DVC0_VALUE             );
  ret = (readExtAddrSPI(FS_LBI)           == CW_FS_LBI_VALUE              );
  ret = (readExtAddrSPI(FS_PFD)           == CW_FS_PFD_VALUE              );
  ret = (readExtAddrSPI(FS_PRE)           == CW_FS_PRE_VALUE              );
  ret = (readExtAddrSPI(FS_REG_DIV_CML)   == CW_FS_REG_DIV_CML_VALUE      );
  ret = (readExtAddrSPI(FS_SPARE)         == CW_FS_SPARE_VALUE            );
  ret = (readExtAddrSPI(FS_VCO4)          == CW_FS_VCO4_VALUE             );
  ret = (readExtAddrSPI(FS_VCO3)          == CW_FS_VCO3_VALUE             );
  ret = (readExtAddrSPI(FS_VCO2)          == CW_FS_VCO2_VALUE             );
  ret = (readExtAddrSPI(FS_VCO1)          == CW_FS_VCO1_VALUE             );
  ret = (readExtAddrSPI(FS_VCO0)          == CW_FS_VCO0_VALUE             );
  ret = (readExtAddrSPI(GBIAS6)           == CW_GBIAS6_VALUE              );
  ret = (readExtAddrSPI(GBIAS5)           == CW_GBIAS5_VALUE              );
  ret = (readExtAddrSPI(GBIAS4)           == CW_GBIAS4_VALUE              );
  ret = (readExtAddrSPI(GBIAS3)           == CW_GBIAS3_VALUE              );
  ret = (readExtAddrSPI(GBIAS2)           == CW_GBIAS2_VALUE              );
  ret = (readExtAddrSPI(GBIAS1)           == CW_GBIAS1_VALUE              );
  ret = (readExtAddrSPI(GBIAS0)           == CW_GBIAS0_VALUE              );
  ret = (readExtAddrSPI(IFAMP)            == CW_IFAMP_VALUE               );
  ret = (readExtAddrSPI(LNA)              == CW_LNA_VALUE                 );
  ret = (readExtAddrSPI(RXMIX)            == CW_RXMIX_VALUE               );
  ret = (readExtAddrSPI(XOSC5)            == CW_XOSC5_VALUE               );
  ret = (readExtAddrSPI(XOSC4)            == CW_XOSC4_VALUE               );
  ret = (readExtAddrSPI(XOSC3)            == CW_XOSC3_VALUE               );
  ret = (readExtAddrSPI(XOSC2)            == CW_XOSC2_VALUE               );
  ret = (readExtAddrSPI(XOSC1)            == CW_XOSC1_VALUE               );
  ret = (readExtAddrSPI(XOSC0)            == CW_XOSC0_VALUE               );
  ret = (readExtAddrSPI(ANALOG_SPARE)     == CW_ANALOG_SPARE_VALUE        );
  ret = (readExtAddrSPI(PA_CFG3)          == CW_PA_CFG3_VALUE             );
  ret = (readExtAddrSPI(WOR_TIME1)        == CW_WOR_TIME1_VALUE           );
  ret = (readExtAddrSPI(WOR_TIME0)        == CW_WOR_TIME0_VALUE           );
  ret = (readExtAddrSPI(WOR_CAPTURE1)     == CW_WOR_CAPTURE1_VALUE        );
  ret = (readExtAddrSPI(WOR_CAPTURE0)     == CW_WOR_CAPTURE0_VALUE        );
  ret = (readExtAddrSPI(BIST)             == CW_BIST_VALUE                );
  ret = (readExtAddrSPI(DCFILTOFFSET_I1)  == CW_DCFILTOFFSET_I1_VALUE     );
  ret = (readExtAddrSPI(DCFILTOFFSET_I0)  == CW_DCFILTOFFSET_I0_VALUE     );
  ret = (readExtAddrSPI(DCFILTOFFSET_Q1)  == CW_DCFILTOFFSET_Q1_VALUE     );
  ret = (readExtAddrSPI(DCFILTOFFSET_Q0)  == CW_DCFILTOFFSET_Q0_VALUE     );
  ret = (readExtAddrSPI(IQIE_I1)          == CW_IQIE_I1_VALUE             );
  ret = (readExtAddrSPI(IQIE_I0)          == CW_IQIE_I0_VALUE             );
  ret = (readExtAddrSPI(IQIE_Q1)          == CW_IQIE_Q1_VALUE             );
  ret = (readExtAddrSPI(IQIE_Q0)          == CW_IQIE_Q0_VALUE             );
  ret = (readExtAddrSPI(RSSI1)            == CW_RSSI1_VALUE               );
  ret = (readExtAddrSPI(RSSI0)            == CW_RSSI0_VALUE               );
  ret = (readExtAddrSPI(MARCSTATE)        == CW_MARCSTATE_VALUE           );
  ret = (readExtAddrSPI(LQI_VAL)          == CW_LQI_VAL_VALUE             );
  ret = (readExtAddrSPI(PQT_SYNC_ERR)     == CW_PQT_SYNC_ERR_VALUE        );
  ret = (readExtAddrSPI(DEM_STATUS)       == CW_DEM_STATUS_VALUE          );
  ret = (readExtAddrSPI(FREQOFF_EST1)     == CW_FREQOFF_EST1_VALUE        );
  ret = (readExtAddrSPI(FREQOFF_EST0)     == CW_FREQOFF_EST0_VALUE        );
  ret = (readExtAddrSPI(AGC_GAIN3)        == CW_AGC_GAIN3_VALUE           );
  ret = (readExtAddrSPI(AGC_GAIN2)        == CW_AGC_GAIN2_VALUE           );
  ret = (readExtAddrSPI(AGC_GAIN1)        == CW_AGC_GAIN1_VALUE           );
  ret = (readExtAddrSPI(AGC_GAIN0)        == CW_AGC_GAIN0_VALUE           );
  ret = (readExtAddrSPI(CFM_RX_DATA_OUT)  == CW_CFM_RX_DATA_OUT_VALUE     );
  ret = (readExtAddrSPI(CFM_TX_DATA_IN)   == CW_CFM_TX_DATA_IN_VALUE      );
  ret = (readExtAddrSPI(ASK_SOFT_RX_DATA) == CW_ASK_SOFT_RX_DATA_VALUE    );
  ret = (readExtAddrSPI(RNDGEN)           == CW_RNDGEN_VALUE              );
  ret = (readExtAddrSPI(MAGN2)            == CW_MAGN2_VALUE               );
  ret = (readExtAddrSPI(MAGN1)            == CW_MAGN1_VALUE               );
  ret = (readExtAddrSPI(MAGN0)            == CW_MAGN0_VALUE               );
  ret = (readExtAddrSPI(ANG1)             == CW_ANG1_VALUE                );
  ret = (readExtAddrSPI(ANG0)             == CW_ANG0_VALUE                );
  ret = (readExtAddrSPI(CHFILT_I2)        == CW_CHFILT_I2_VALUE           );
  ret = (readExtAddrSPI(CHFILT_I1)        == CW_CHFILT_I1_VALUE           );
  ret = (readExtAddrSPI(CHFILT_I0)        == CW_CHFILT_I0_VALUE           );
  ret = (readExtAddrSPI(CHFILT_Q2)        == CW_CHFILT_Q2_VALUE           );
  ret = (readExtAddrSPI(CHFILT_Q1)        == CW_CHFILT_Q1_VALUE           );
  ret = (readExtAddrSPI(CHFILT_Q0)        == CW_CHFILT_Q0_VALUE           );
  ret = (readExtAddrSPI(GPIO_STATUS)      == CW_GPIO_STATUS_VALUE         );
  ret = (readExtAddrSPI(FSCAL_CTRL)       == CW_FSCAL_CTRL_VALUE          );
  ret = (readExtAddrSPI(PHASE_ADJUST)     == CW_PHASE_ADJUST_VALUE        );
  ret = (readExtAddrSPI(SERIAL_STATUS)    == CW_SERIAL_STATUS_VALUE       );
  ret = (readExtAddrSPI(MODEM_STATUS1)    == CW_MODEM_STATUS1_VALUE       );
  ret = (readExtAddrSPI(MODEM_STATUS0)    == CW_MODEM_STATUS0_VALUE       );
  ret = (readExtAddrSPI(MARC_STATUS1)     == CW_MARC_STATUS1_VALUE        );
  ret = (readExtAddrSPI(MARC_STATUS0)     == CW_MARC_STATUS0_VALUE        );
  ret = (readExtAddrSPI(PA_IFAMP_TEST)    == CW_PA_IFAMP_TEST_VALUE       );
  ret = (readExtAddrSPI(FSRF_TEST)        == CW_FSRF_TEST_VALUE           );
  ret = (readExtAddrSPI(PRE_TEST)         == CW_PRE_TEST_VALUE            );
  ret = (readExtAddrSPI(PRE_OVR)          == CW_PRE_OVR_VALUE             );
  ret = (readExtAddrSPI(ADC_TEST)         == CW_ADC_TEST_VALUE            );
  ret = (readExtAddrSPI(DVC_TEST)         == CW_DVC_TEST_VALUE            );
  ret = (readExtAddrSPI(ATEST)            == CW_ATEST_VALUE               );
  ret = (readExtAddrSPI(ATEST_LVDS)       == CW_ATEST_LVDS_VALUE          );
  ret = (readExtAddrSPI(ATEST_MODE)       == CW_ATEST_MODE_VALUE          );
  ret = (readExtAddrSPI(XOSC_TEST1)       == CW_XOSC_TEST1_VALUE          );
  ret = (readExtAddrSPI(XOSC_TEST0)       == CW_XOSC_TEST0_VALUE          );
